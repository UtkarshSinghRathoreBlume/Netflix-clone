...
const SearchRow = ({results}) => {
  ...
  return (
    <div className='bg-black '>
          {!isGptSearch ? <div className='searchRow__container my-32 h-full text-white w-full flex flex-wrap flex-grow'>
                {results.map((data) => {
                  return(
            ...
            <div className='searchRow__container my-32 h-full text-white w-full flex flex-wrap flex-grow'>
              {console.log(gptMovies)}
               {showLoader ||  gptMovies == [] ? <div>Loading...</div> : gptMovies?.map(res => {
